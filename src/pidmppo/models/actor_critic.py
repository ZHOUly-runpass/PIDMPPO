...
class ActorCritic(nn.Module):
    ...
    def forward(
        ...
        log_std = self.actor_log_std(shared).clamp(-5.0, 2.0)
    ...
    @staticmethod
    def sample_action(mean: torch.Tensor, log_std: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    def evaluate_action(
        ...
        distribution = Normal(mean, log_std.exp())
        log_prob = ActorCritic._squashed_log_prob(distribution, raw, clipped)
        # Base-normal entropy is a stable estimator used for the PPO bonus.
